...
class cvrpProblem():
    """ This is designed to find solutions of IP CVRPs """
    ...
    def set_depot(self, graph):
        """ Set the depot vertex for the problem """

        self.depot_node = graph.graph[0]["departure_node"]

    def set_vehicle_capacity(self, graph):
        """ Set the vehicle capacity """

        self.vehicle_capacity = graph[0]["capacity"]

    def set_vehicles_homogeneous(self, graph):
        """ Set the vehicles, assuming heterogeneity """

        self.vehicles = len(graph.graph)

    def check_vehicles(self):
        """ Make sure the vehicles are properly specified """

        if self.vehicles is None:
            raise ValueError("Vehicles not correctly specified!")
