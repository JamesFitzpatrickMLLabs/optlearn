...
from optlearn.io.vrp import vrp_xml_utils
from optlearn.io.vrp import vrp_txt_utils
...
def miles_to_kilometers(miles):
    """ Convert the given measurement from miles to kiometers. Ubelieveable this is necessary. """
...
def per_mile_to_per_kilometer(per_mile):
    """ Convert the given measurement from per miles to per kilometers """

    per_kilometer = per_mile / 1.60934
...
def add_vehicle_count(graph, filename, filename_parser=None):
    ...
    if filename_parser is None:
        ...
        graph.graph["num_vehicles"] = filename_parser(filename)
...
def duplicate_vehicle(graph, vehicle_key): 
   ...
   graph.graph["fleet"][new_vehicle_key] = copy.deepcopy(graph.graph["fleet"][vehicle_key])
...
def check_fleet(graph):
    ...
    if num_vehicles is not None:
        if len(vehicle_keys) == 1 and num_vehicles > 1:
            for num in range(num_vehicles):
                duplicate_vehicle(graph, vehicle_keys[0])
    return graph


def get_node_types(graph, problem_type="cvrp"):
    """ Get the depot set for a graph of the given problem type """

    type_keys = _vehicle_node_types[problem_type]
    type_dict = process_utils.get_node_types_dict(graph, graph.nodes)
    for (key, value) in type_keys.items():
        type_dict[key] = type_dict.pop(value)

    return type_dict


def add_node_type_info(graph, problem_type):
    """ Parse and add the node type info to the given graph metadata """

    node_types = get_node_types(graph, problem_type)
    graph.graph["node_types"] = node_types

    return graph


def read_vrp_problem(filename, problem_type, rounding=0):
    ...
    graph = add_distance_edges(graph, info_dict,
                               metric=_vehicle_metrics.get(problem_type), rounding=rounding)
    ...
    graph = add_node_type_info(graph, problem_type)
    
    return graph


def metricise_average_velocities(graph):
    """ Convert fuel average velocities from miles to kilometers """

    fleet = graph.graph["fleet"]
    for vehicle_key in fleet.keys():
        metric_velocity = miles_to_kilometers(fleet[vehicle_key]["average_velocity"])
        fleet[vehicle_key]["average_velocity"] = metric_velocity

    return graph


def metricise_fuel_consumption_rates(graph):
    """ Convert fuel tank capacities from miles to kilometers """

    fleet = graph.graph["fleet"]
    for vehicle_key in fleet.keys():
        metric_consumption = per_mile_to_per_kilometer(fleet[vehicle_key]["fuel_consumption_rate"])
        fleet[vehicle_key]["fuel_consumption_rate"] = metric_consumption
...
def read_gvrp_problem(filename):
    ...
    graph = metricise_fuel_consumption_rates(graph)
    graph = metricise_average_velocities(graph)
    
    return graph


def read_evrpnl_problem(filename, rounding=16):
    """ Read an  evrpnl problem file into a networkx graph """

    graph = read_vrp_problem(filename, problem_type="evrpnl", rounding=rounding)
